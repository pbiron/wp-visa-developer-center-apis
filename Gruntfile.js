	...
	grunt.initConfig( {
        ...
        copy: {
            release: {
                expand: true,
                src: [
                	'plugin.php', 'uninstall.php', 'readme.txt', 'assets/**', 'includes/**', 'vendor/**',
                	// for some reason, the includes/certs/.htaccess file is not copied.
                	// so for now, I'll have to add it to the release zip manually :-(
                    '!assets/css/**/*.scss', '!vendor/bin',
                    '!includes/class-project-config.php',
                    '!includes/certs/**/*.pem','!includes/certs/**/*.crt',
                ],
                dest: '<%= pkg.name %>',
            },
            node_modules: {
